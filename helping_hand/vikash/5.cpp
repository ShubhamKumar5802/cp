...
long long dp(int n, vector<ll>&A, int k, vector<vector<ll>>&memo, int i){
	if(k==0 or i == 0)return 0;
	if(k%2){
		return max(-A[i - 1] + dp(n, A, k, memo, i - 1), A[i - 1] + dp(n, A, k - 1, memo, i - 1));
	}else{
		return max(A[i - 1] + dp(n, A, k, memo, i - 1), -A[i - 1] + dp(n, A, k - 1, memo, i - 1));
	}
}
long long maxii(int n, vector<ll>&A, int k){
	vector<vector<ll>>memo(n + 1, vector<ll>(n  +1, -1));
	return dp(n, A, k, memo, n);
}
