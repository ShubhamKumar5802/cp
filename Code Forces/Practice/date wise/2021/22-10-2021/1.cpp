...
int main(){
   ...
   ll f_score=accumulate(f.begin(),f.end(),0ll);
   ll s_score=accumulate(s.begin(),s.end(),0ll);
   if(f_score+s_score>0){
       cout<<"first"<<endl;
   }else if(s_score+f_score<0){
       cout<<"second"<<endl;
   }else{
       int f_count=f.size(),s_count=s.size();
       int counter=min(f_count,s_count);
