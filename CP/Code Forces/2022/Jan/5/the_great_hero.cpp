...
int main() {
	...
	cin >> __;
	while (__--) {
		ll h_attack, h_health, n;
		cin >> h_attack >> h_health >> n;
		vpll v(n);
		rep(i, 0, n)cin >> v[i].first;
		rep(i, 0, n)cin >> v[i].second;
		bool flag = true;
		for (int i = 0; i < n; i++) {
			ll fight = v[i].second % h_attack == 0 ? v[i].second / h_attack : v[i].second / h_attack + 1;
			if (h_health > (fight - 1)*v[i].first) {
				h_health -= (fight) * v[i].first;
			} else {
				flag = false;
				break;
			}
