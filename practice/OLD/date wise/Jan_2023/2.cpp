...
long long power(long long x, long long n) {x = x % MOD; if (x == 0 and n == 0)return 1; if (x == 0)return 0; long long result = 1; while (n > 0) {if (n & 1)result = (result * x) % MOD; n = n >> 1; x = (x * x) % MOD;} return result;}
