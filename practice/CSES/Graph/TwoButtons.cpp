...
void solve()
{
	...
	while (pq.size()) {
		...
		if (2 * curr.ss < sz and visited[2 * curr.ss] > curr.ff + 1) {
			visited[2 * curr.ss] = curr.ff + 1;
			pq.push({curr.ff + 1, 2 * curr.ss});
		}
		if (curr.ss > 1 and visited[curr.ss - 1] > curr.ff + 1) {
			visited[curr.ss - 1] = curr.ff + 1;
			pq.push({curr.ff + 1, curr.ss - 1});
		}
	}
	cout << "-1" << endl;
}
