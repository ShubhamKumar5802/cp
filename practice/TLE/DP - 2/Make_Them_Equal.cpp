...
void pc() {
	...
	while (q.size()) {
		...
		for (int i = 1; i <= curr.F; i++) {
			ll nnode = curr.F + (curr.F / i);
			if (nnode > 1001)continue;
			if (dist[nnode] > curr.S + 1ll) {
				dist[nnode] = curr.S + 1ll;
				pll p = {nnode, curr.S + 1ll};
				q.push(p);
			}
		}
	}
...
void solve()
{
	...
	rep(i, 1, n + 1) {
		rep(j, 0, k + 1) {
			if (j >= dist[b[i - 1]]) {
				dp[i][j] = max((int)c[i - 1] + dp[i - 1][j - dist[b[i - 1]]], dp[i - 1][j]);
			} else {
				dp[i][j] = dp[i - 1][j];
			}
		}
