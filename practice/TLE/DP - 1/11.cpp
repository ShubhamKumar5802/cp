...
vector<int>isPrime;
template<typename T>
void print1D(vector<T> nums) {for (int i = 0; i < sz(nums) - 1; i++)cout << fixed << setprecision(3) << nums[i] << " "; if (sz(nums))cout << nums[nums.size() - 1];}
template <typename T>
...
void solve()
{
	...
	dp[0][0] = 1;
	rep(i, 1, n + 1)dp[i][0] = dp[i - 1][0] * (1 - v[i - 1]);
	rep(i, 1, n + 1) {
		rep(j, 1, i + 1) {
